...
public class Stemmer {
	...
	public String toString() {
		...
		String res = new String(b, 0, i_end);
		int len = res.length();
		if (res.equals("best") || res.equals("better"))
			return "good";
		else if (res.equals("worse") || res.equals("worst"))
			return "bad";
		else if (res.equals("farther") || res.equals("further"))
			return "far";
		else if (res.equals("less") || res.equals("least"))
			return "little";
		else if (res.equals("more") || res.equals("most"))
			return "many";

		// base degree for an adjective should be at least 3 chars long

		else if (res.endsWith("er")) {
			if (len >= 5) {
				if (len >= 6) {
					// might be something like "bigger"
					if (notVowel(res.charAt(len - 3))
							&& res.charAt(len - 3) == res.charAt(len - 4))
						return res.substring(0, len - 3);
					// base degree ends with "y"
					else if (notVowel(res.charAt(len - 4))
							&& res.charAt(len - 3) == 'i')
						return res.substring(0, len - 3) + "y";
				}
				return res.substring(0, len - 2);
			}
		} else if (res.endsWith("est")) {
			if (len >= 6) {
				if (len >= 7) {
					// might be something like "biggest"
					if (notVowel(res.charAt(len - 4))
							&& res.charAt(len - 4) == res.charAt(len - 5))
						return res.substring(0, len - 4);
					// base degree ends with "y"
					else if (notVowel(res.charAt(len - 5))
							&& res.charAt(len - 4) == 'i')
						return res.substring(0, len - 4) + "y";
				}
				return res.substring(0, len - 3);
			}
		}
